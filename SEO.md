...
현재 GitHub 하위경로 등록은 실제 등록 화면에서 거부되었습니다(호스트 단위 등록 요구). 현재 계정의 사이트 목록은 비어 있었습니다. 노출·클릭 리포트를 사용하려면 자체 도메인 또는 승인된 호스트 루트 소유확인이 필요합니다. 기존 사이트나 호스트 루트는 이번 작업에서 바꾸지 않습니다. 도메인을 구매하지 않아도 자동 수집과 IndexNow는 가능하지만, 현재 경로만 별도 사이트로 등록했다고 표시하면 안 됩니다.

도메인을 정한 뒤 소유확인 → 사이트맵 제출 → 주요 URL 수집요청 → 수집·색인·노출 보고서 확인 순서로 진행하세요. 등록용 meta를 받으면 site.config.json.naverVerification에 저장하고 빌드합니다. 자체 도메인을 쓰더라도 GitHub Pages 호스팅을 유지할 수 있습니다. 주소 변경 때 canonical·내부 링크·사이트맵·IndexNow 키 범위를 함께 재검수해야 합니다.
