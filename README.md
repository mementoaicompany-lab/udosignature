...
GitHub Pages의 Custom domain은 `udosignature.com`, 배포 소스는 `main /docs`입니다. DNS의 루트 A 레코드는 GitHub Pages의 `185.199.108.153`, `185.199.109.153`, `185.199.110.153`, `185.199.111.153`을 사용합니다. `www`는 `mementoaicompany-lab.github.io` CNAME으로 연결하고, GitHub Pages에서 기본 주소로 리디렉션합니다. 카페24가 허용하는 레코드 수에 제약이 있으면 실제 설정과 이유를 연결 결과 문서에 기록합니다. `guide.udosignature.com`은 이번 연결 대상에 포함하지 않으며, 기존 고객 안내 링크는 `https://mementoaicompany-lab.github.io/coconara/`를 유지합니다.
